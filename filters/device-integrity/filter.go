...
import (
	...
	"github.com/muzzapp/skipper/pkg/appattest/android"
	"github.com/muzzapp/skipper/pkg/appattest/ios"
...
func (a attestationFilter) Request(ctx filters.FilterContext) {
	...
	if existingAppAttestation != nil && existingAppAttestation.KeyID != "" {
		existingKeyID = &existingAppAttestation.KeyID
	}

	// If we have the X-Assertation header but not the Authorization header, that's fine, we'll only do the assertation
	...
	// Set the challenge response and KeyID we received
	existingAppAttestation.ChallengeResponse = authorizationHeader
	existingAppAttestation.KeyID = encodedKeyID
	err := a.repo.UpdateAttestationForUDID(existingAppAttestation)
	if err != nil {
		a.logger.Error("update challenge response", "err", err)
	}
	...
	switch {
	case isAndroid:
		googlePlay := android.NewGooglePlayIntegrityServiceClient(a.logger)

		verdict := googlePlay.Validate(challengeResponse, serverNonce, existingAppAttestation)
		err = a.repo.UpdateAttestationForUDID(existingAppAttestation)
		if err != nil {
			a.logger.Error("update challenge response", "err", err)
		}

		if verdict == appattest.IntegritySuccess {
			return // All good, proceed
		}

		if verdict == appattest.IntegrityUnevaluated {
	...
	case isIOS:
		...
		appStore := ios.NewAppStoreIntegrityServiceClient(a.logger)

		verdict, validateErr := appStore.ValidateAttestation(
			existingAppAttestation,
			authorizationHeader,
		...
		if verdict == appattest.IntegrityFailure {
			sendErrorResponse(ctx, http.StatusForbidden, "Integrity check failed")
			return
		}
		...
		verdict, validateErr = appStore.ValidateAssertation(
			existingAppAttestation,
			encodedAssertation,
			encodedKeyID,
		)
...
func (a attestationFilter) generateChallenge(
	deviceUDID string,
	platform Platform,
